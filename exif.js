...
export default {
    ...
    async fixExif(file) {
        ...
        const exif = {
            title: json.title,
            "gps:latitude":
                json.geoData?.latitude || json.geoDataExif?.latitude || null,
            "gps:longitude":
                json.geoData?.longitude || json.geoDataExif?.longitude || null,
            GPSLongitude:
                json.geoData?.longitude || json.geoDataExif?.longitude || null,
            GPSLatitude:
                json.geoData?.latitude || json.geoDataExif?.latitude || null,
            Description:
                ...
                "From Google Photos - https://github.com/jodacame/google-takeout-fixer",
        ...
        if (metadata) {
            try {
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].CreateDate
                    );
                if (!taken)
                    taken = this.getDateFromMetadata(
                ...
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].ModifyDate
                    );
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].FileModifyDate
                    );
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].FileCreateDate
                    );
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].GPSDateTime
                    );
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].GPSDateStamp
                    );
                if (!taken)
                    taken = this.getDateFromMetadata(
                        metadata.data[0].MediaCreateDate
                    );
        ...
        if (metadata) {
            try {
                if (
                    metadata.data[0].GPSLatitude &&
                    metadata.data[0].GPSLongitude
                ) {
                    delete exif["gps:latitude"];
                    delete exif["gps:longitude"];
                    delete exif.GPSLongitude;
        ...
        if (typeof taken === "number" || typeof taken === "string")
            taken = new Date(taken);
